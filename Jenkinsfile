...
pipeline {
  ...
  stages {
    stage('Build Desktop') {
      steps {
        sh 'rm -f fly_over_river-*.tar.gz fly_over_river-*.zip fly_over_river*.apk'
  ...
  post {
    success {
      archiveArtifacts artifacts: 'fly_over_river*.tar.gz,fly_over_river*.zip,fly_over_river*.apk'
    }
    regression {
